...
Plugins can define **lifecycle hooks** — shell scripts that run at specific points during deployment. Hooks can auto-resolve derived secrets, run interactive onboarding, install server-side tools, and perform pre-launch configuration. See the [Plugin Hooks guide](/guides/plugin-hooks) for details.
