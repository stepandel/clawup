...
1. `clawup init` generates a `clawup.yaml` with `${env:VAR}` references in the `secrets` section
2. A `.env.example` file is generated alongside the manifest showing all required variables
3. You copy `.env.example` to `.env` and fill in values
4. On subsequent runs, `clawup init` loads `.env` and skips prompts for any pre-populated values
...
<Note>
The `.env` file is automatically added to `.gitignore` by `clawup init`. Actual secret values are stored encrypted in Pulumi config — the `.env` file is a convenience for skipping interactive prompts.
</Note>
...
The `.env.example` file is generated automatically by `clawup init`. It lists all required env var names with empty values, organized by scope:
...
By default, `clawup init` looks for `.env` in the project root. Override with:

```bash
clawup init --env-file /path/to/my/.env
```

## Validation

Clawup validates well-known secret formats during init:
...
## Managing Secrets Post-Setup

After initial setup, manage secrets without re-running init:
