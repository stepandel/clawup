...
Each camelCase key is converted to `SCREAMING_SNAKE_CASE` and prefixed with the agent's role for the env var name. During `clawup init`, the wizard checks `.env` for pre-populated values and prompts for any that are missing.
