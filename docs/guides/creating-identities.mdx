...
<Tip>
The fastest way to create a new identity is with [`army-create`](https://github.com/stepandel/army-create) — run `npx army-create` to scaffold all required files interactively. You can also copy the [example identity](https://github.com/stepandel/clawup/tree/main/examples/identity) and modify it, or see the [multi-agent example](https://github.com/stepandel/clawup/tree/main/examples/multi-agent) for a complete multi-agent deployment.
</Tip>
...
Each camelCase key is converted to `SCREAMING_SNAKE_CASE` and prefixed with the agent's role for the env var name. Add the values to your `.env` file — `clawup deploy` validates that all required secrets are present and reports any that are missing.
...
- **Start with `army-create`.** Run `npx army-create` to scaffold a new identity interactively ([GitHub](https://github.com/stepandel/army-create) · [npm](https://www.npmjs.com/package/army-create)). Or copy `examples/identity/` for a single agent, or `examples/multi-agent/` for a full PM + Engineer + Tester fleet.
