...
Discovers local identity directories (subdirectories containing `identity.yaml`) and scaffolds a manifest with sensible defaults. If you don't have identities yet, create one first with [`army-create`](https://github.com/stepandel/army-create) (`npx army-create`).
...
3. Run [`clawup deploy`](/cli/deploy) to validate secrets, configure Pulumi, and deploy your agents
