---
title: "init"
description: "Interactive setup wizard for configuring your agent fleet"
---

# agent-army init

Interactive setup wizard that walks you through configuring your stack, secrets, and agents.

## Usage

```bash
agent-army init [options]
```
...
### 6. Agent Selection
Pick from available presets and optionally customize each agent.
...
agent-army init --deploy

# Init, deploy, skip confirmation
agent-army init --deploy --yes
```

## Output

Creates `~/.agent-army/configs/<stack-name>/agent-army.json` with your full manifest.
