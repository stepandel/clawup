...
agent-army.yaml              Pulumi Stack          Cloud Provider
   (Manifest)          ────▶  (TypeScript)   ────▶  (AWS / Hetzner)
        │                          │
        ▼                          ▼
identity.yaml            Per-agent resources:
  (Per-agent)            • Server (Ubuntu 24.04)
  • Model config         • Security group / firewall
  • Plugins & deps       • SSH keypair
  • Skills               • Gateway token
  • Workspace files      • Cloud-init script
```
