---
title: "How It Works"
description: "End-to-end walkthrough of how Clawup provisions and runs your agents"
---

Clawup uses [Pulumi](https://pulumi.com) to provision cloud infrastructure as code, deploying each agent on a dedicated Ubuntu server with persistent storage, secure networking, and all the tools defined by its identity.
...
5. **Run resolve hooks** — Swarm, identity, and plugin resolve hooks run on your machine to auto-derive secrets. Most-specific-wins on key conflicts (plugin > identity > swarm).
6. **Provision resources** — Pulumi creates infrastructure for each agent: server, firewall, SSH key, and injects a cloud-init bootstrap script as user data.
7. **Cloud-init runs** — On first boot, each server executes the bootstrap sequence (see below) to install tools, configure OpenClaw, and join the Tailscale mesh. Lifecycle hooks (postProvision, preStart) execute in order: swarm → identity → plugin.
8. **Show outputs** — The CLI displays each agent's Tailscale URL, public IP, and instance ID.
...
### 9. PostProvision Hooks

- Run swarm-level `postProvision` hook (from `clawup.yaml`)
- Run identity-level `postProvision` hook (from `identity.yaml`)
- Run plugin-level `postProvision` hooks (from plugin manifests)
- Hooks execute sequentially: swarm → identity → plugin. See [Lifecycle Hooks](/guides/hooks) for details.

### 10. Identity File Injection
...
### 11. OpenClaw Configuration & Plugins
...
- Run swarm-level `preStart` hook (from `clawup.yaml`)
- Run identity-level `preStart` hook (from `identity.yaml`)
- Run plugin-level `preStart` hooks (from plugin manifests)
- Hooks execute sequentially: swarm → identity → plugin. See [Lifecycle Hooks](/guides/hooks) for details.
