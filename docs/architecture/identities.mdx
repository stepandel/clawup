...
Clawup discovers identities from local directories (subdirectories containing `identity.yaml`) or Git repos. The easiest way to create a new identity is with [`army-create`](https://github.com/stepandel/army-create) — run `npx army-create` to scaffold all required files interactively. You can also start from the [example identity](https://github.com/stepandel/clawup/tree/main/examples/identity) or the [multi-agent example](https://github.com/stepandel/clawup/tree/main/examples/multi-agent) for a complete 3-agent fleet with cross-agent ticket workflows.
...
To create an agent with a completely custom role, use [`army-create`](https://github.com/stepandel/army-create) to scaffold a new identity:

```bash
npx army-create
```

This interactively generates all required files — `identity.yaml`, workspace files, and skill stubs. See the [Creating Identities](/guides/creating-identities) guide for a full walkthrough, or start from the [example identity](https://github.com/stepandel/clawup/tree/main/examples/identity) or the [multi-agent example](https://github.com/stepandel/clawup/tree/main/examples/multi-agent).
